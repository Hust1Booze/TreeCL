...
def run_episode(oracle_type, instance,  save_dir, save_dir_svm, device):
    ...
    oracle_ns.set_LP_feature_recorder(LPFeatureRecorder(model, device, optsol))
...
if __name__ == "__main__":
    ...
    for data_partition in data_partitions:
        ...
        save_dir = os.path.join(os.path.dirname(__file__), f'./data/test/{problem}/{data_partition}')
        save_dir_svm = os.path.join(os.path.dirname(__file__), f'./data_svm/test/{problem}/{data_partition}')
        ...
        print(f"instance path : {instances[0]}")
